...
class MultiHeadAttn_Root_Enhence(nn.Module):
    ...
    def forward(self, x, adj, mask_nonzero):
        """
        ...
        if self.root_enhance:
            # #bool_zeromask
            # fea_mask = torch.repeat_interleave(mask, x.size()[-1], dim=-1)
            # root = torch.zeros_like(x, device=x.device)
            # root = torch.where(fea_mask, root, x)
            #index_nonzeromask
...
class TransformerLayer(nn.Module):
    def __init__(self, h_in, n_head, h_hid, dropout, after_norm, dropout_attn, attn_scale,root_enhance):
        ...
        self.ffn = nn.Sequential(nn.Linear(h_in, h_hid, bias=False),
                                 nn.LeakyReLU(),
                                 nn.Dropout(dropout),
                                 nn.Linear(h_hid, h_in, bias=False),
                                 nn.Dropout(dropout))
