...
from Claim_Guided.ClaimH import ClaimH
from Claim_Guided.RvNN import RvNN_BU, RvNN_TD_Leaf_Nodes, RvNN_TD_All_Nodes
from Claim_Guided.Text_CNN import Text_CNN
from model.Model_layers import Base
...
class ClaimH_NET(object):
    def __init__(self, args, device):
        ...
        # self.model = ClaimH(self.num_heads, self.f_in,
        #                            self.h_DUGAT,self.f_hid, self.dropout, self.num_class)
        # self.model = RvNN_TD_Leaf_Nodes(self.f_in,self.f_hid, self.num_class)
        # self.model = Text_CNN(self.num_filter, self.f_in, self.f_hid, self.num_posts, self.dropout)
        self.model = Base(self.f_in, 1,  2, 200, 100, False, self.num_posts)
    ...
    def train_epoch(self, datapath, start_epoch):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            with torch.no_grad():
                val_loss, val_acc_dict = self.eval(val_loader, epoch)
            end_time = time.clock()
        ...
        with torch.no_grad():
            ...
            test_loss, test_acc_dict = self.eval(test_loader, start_epoch)
