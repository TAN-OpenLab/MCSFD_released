...
from Claim_Guided.ClaimH import ClaimH, ClaimH_CCFD
from Claim_Guided.RvNN import RvNN_BU, RvNN_TD_Leaf_Nodes, RvNN_TD_All_Nodes, RvNN_TD_Leaf_Nodes_CCFD
from Claim_Guided.Text_CNN import Text_CNN_CCFD
...
class CliamH_CCFD_NET(object):
    def __init__(self, args, device):
        ...
        # model parameters
        self.filter_num = args.num_filter
        self.f_in, self.f_hid = args.text_embedding
        ...
        # self.model = ClaimH_CCFD(self.num_heads, self.f_in,
        #                            self.h_DUGAT,self.f_hid, self.dropout, self.num_posts)
        # self.model = RvNN_TD_Leaf_Nodes_CCFD(self.f_in, self.f_hid, self.num_posts)
        self.model = Text_CNN_CCFD(self.filter_num, self.f_in, self.f_hid, self.num_posts, self.dropout)
        ...
        self.optimizer = torch.optim.SGD( self.model.parameters(), lr= self.lr, momentum= 0.9, weight_decay=1e-2)
    ...
    def train_epoch(self, datapath, start_epoch):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            with torch.no_grad():
                val_loss, all_celoss, xc_celoss, cent_loss, recoverey_loss, val_acc_dict = self.evaluation(val_loader,
                                                                                                           epoch)
            end_time = time.clock()
            print(val_loss, all_celoss, xc_celoss, cent_loss, recoverey_loss, val_acc_dict['model_DI']['acc'])

            if all_celoss < loss_all_check and acc_all_check <= val_acc_dict['model']['acc']:  # or
                acc_all_check = val_acc_dict['model']['acc']
                loss_all_check = all_celoss
                self.checkpoint = epoch
                self.save(self.model_path, epoch, source=True)
                patience = self.patience

            if val_acc_dict['model_DI']['acc'] >= acc_trans_check:
                if val_acc_dict['model_DI']['acc'] in loss_trans_check.keys():
                    if cent_loss <= loss_trans_check[val_acc_dict['model_DI']['acc']]:
                        loss_ce_check = xc_celoss
                        acc_trans_check = val_acc_dict['model_DI']['acc']
                        loss_trans_check[val_acc_dict['model_DI']['acc']] = cent_loss
        ...
        with torch.no_grad():
            ...
            test_loss, all_celoss, xc_celoss, cent_loss, recoverey_loss, test_acc_dict = self.evaluation(test_loader,
                                                                                                         start_epoch)
    ...
    def train_batch(self, epoch, nonrumor_loader, rumor_loader):
        ...
        for iter, (Nonrumors, Rumors) in enumerate(iterloader):
            ...
            preds, preds_xc, preds_xs, xc, xs, x_rec, fgate = self.model(x, A)
            loss, (all_celoss, xc_celoss, cent_loss, rec_loss) = self.loss_wrapper(
                [preds, preds_xc,preds_xs, xc, xs, x_rec, fgate], [y, x])
    ...
    def evaluation(self, dataloader, epoch):
        ...
        for iter, sample in enumerate(dataloader):
            ...
            preds, preds_xc, preds_xs, xc, xs, x_rec, fgate = self.model(x, A)
            loss, (all_celoss, xc_celoss, cent_loss, rec_loss) = self.loss_wrapper(
                [preds, preds_xc,preds_xs, xc, xs, x_rec, fgate],
                [y, x])
            mean_loss += loss.item()
            Mall_ce_loss += all_celoss
            XC_ce_loss += xc_celoss
